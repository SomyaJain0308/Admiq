...
from backend.services.storage_service import upload_file_bytes
from backend.services.document_service import create_document_row
...
async def upload_document(college_id: int, uploaded_by: int = Form(...), file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    doc = create_document_row(db, college_id=college_id, file_name=file.filename, storage_path="", uploaded_by=uploaded_by)
    storage_path = f"{college_id}/{doc.document_id}/{file.filename}"
    upload_file_bytes(storage_path, file_bytes, content_type=file.content_type)
