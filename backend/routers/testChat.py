...
async def test_chat(request: Request, payload: ChatTestRequest, db: Session = Depends(get_db)):
    ...
    inbound = save_inbound_message(db, college_id=payload.college_id, student_id=student.student_id, whatsapp_message_id=f"test-{uuid.uuid4()}", content=payload.message, whatsapp_timestamp=datetime.utcnow(), message_type="text", raw_payload={}, session_id=session.session_id)
    ...
    if model_used not in ["security_block", "error", "budget_exceeded"] and wants_human_handoff:
        flag_low_confidence_query(db, college_id=payload.college_id, student_id=student.student_id, question_message_id=inbound.message_id, answer_message_id=assistant_msg.message_id, similarity_score=best_distance)
