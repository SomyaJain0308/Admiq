...
class StudentProfileUpdate(BaseModel):
    ...
    guardian_involvement: str | None = Field(default=None, description="A short free-text note on parent/guardian involvement in the decision, e.g. 'Father is main decision-maker, mother involved less.' Update this only if this session revealed new or changed information about parent/guardian involvement; otherwise return the existing note unchanged. Null if it has never come up.")
    competing_colleges: list[str] = Field(default_factory=list, description="The FULL current list of other colleges/institutes the student has explicitly named as alternatives they are considering, merging the existing list with any newly mentioned this session. Deduplicate. Empty list if none have been mentioned.")
