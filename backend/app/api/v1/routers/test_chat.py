...
from backend.app.schemas.models import ChatTestRequest, ChatTestResponse
...
async def test_chat(request: Request, payload: ChatTestRequest, db: AsyncSession = Depends(get_db)):
    ...
    assistant_msg = await save_assistant_message(db, college_id=payload.college_id, student_id=student.student_id, content=response_text, sources=sources, session_id=session.session_id)

    if model_used not in ["security_block", "error", "budget_exceeded"] and wants_human_handoff:
        await flag_low_confidence_query(db, college_id=payload.college_id, student_id=student.student_id, question_message_id=inbound.message_id, answer_message_id=assistant_msg.message_id, similarity_score=best_distance)
    if new_session_summary:
        await update_session_summary(db=db, session=session, session_summary=new_session_summary)
