...
def update_document_status(db, college_id: int, document_id: int, status: str, extraction_method: str | None = None, quality_score: float | None = None, num_pages: int | None = None, error: str | None = None) -> models.Document | None:
    doc = db.execute(select(models.Document).where(models.Document.college_id == college_id, models.Document.document_id == document_id).limit(1)).scalars().first()
    if doc is None:
        return None
